/datum/admins/proc/Secrets()
	...
	if(check_rights(R_FUN,0))
		dat += {"
			...
			<A href='?src=[REF(src)];[HrefToken()];secrets=disable'>Make all players disabled</A><BR>
			...
			<A href='?src=[REF(src)];[HrefToken()];secrets=floorlava'>The floor is lava! (DANGEROUS: Not to mention quite pathetic.)</A><BR>
	...
	switch(item)
		...
		if("disable")
			...
			for(var/mob/living/carbon/human/H in GLOB.player_list)
				to_chat(H, "<span class='boldannounce'>You suddenly feel not quite right.</span>")
				H.adjustOrganLoss(ORGAN_SLOT_BRAIN, 60, 80)
			message_admins("[key_name_admin(usr)] disabled everyone. Good job(!)")
